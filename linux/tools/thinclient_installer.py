...
# if we already have usr, opt and var folders, delete those before we begin
delme=["usr","var","opt"]
for i in delme:
 if os.path.exists(i):
  os.unlink(i)
...
os.system("cp %s/thinclient_base1.xml ."%(cmd_path))

# create the License.env file
os.system("python %s/create_license_env.py %s"%(cmd_path,product_key))

# create a run.sh
os.system("python %s/create_run_sh.py %s"%(cmd_path,maya_version))
...
cmd="sudo ./adlmreg -i N %s %s %s 123-12345678 var/opt/Autodesk/Adlm/%s"%(product_key,product_key,version_string,maya_pit_file)
os.system(cmd)
print cmd
cmd="sudo mv /var/opt/Autodesk/Adlm/.config/ProductInformation.pit var/opt/Autodesk/Adlm/.config/ProductInformation.pit"
os.system(cmd)
print cmd
